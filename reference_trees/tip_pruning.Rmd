...
#looney <- get_tips("Looney_unrooted_unedited", "looney_tips.csv")
...
russ_tree <- get_tips("looney_unrooted_unedited", "russulaceae_tips.csv")
```
...
```{r}
test <- read.csv("testing_files/jgi_fams_inorder.csv")
test <- subset(test, select=-c(X))

test <- test %>% group_by(Family) %>%tally()
```
```{r}
...
russ_tree <- drop.tip(russ_tree, c("Lactifluus_longisporus_AV99-197_BB_00.1519", "R_cremeiceps_ISOTYPE_FL", "R_subrubescens_COISOTYPE_FL", "R_brunneipes_ISOTYPE_FL", "R_levyana_ISOTYPE_FL", "R_pinophila_ISOTYPE_FL", "R_vinosorosea_ISOTYPE_FL", "R_albomarginata_ISOTYPE_FL", "R_emeticiformis_ISOTOYPE_FL", "R_fragiloides_ISOTYPE_FL", "R_aff_vesca_FH12106_THA", "R_australirosea_ISOTYPE_FL", "Lacttifluus_edulis_AV99-041", "R_variicolor_ISOTYPE_FL"
))

write.tree(russ_tree, file="russulaceae_backbone1.nwk")
