...
get_fams <- function(trefile) {
  ...
  # manipulate taxonomy information to get family
...
jgi <- get_fams("JGItree.nwk")
li <- get_fams("Lietal_2021.nwk")
```

Get missing family information from NCBI
```{r}
jgi_ids <- jgi$non_unite$Species
test <- get_ids(jgi_ids, db = "ncbi")

for (i in 1:62) {
  test1 <- test$ncbi[i]
  fam <- classification(test1, db = "ncbi")
}
test1 <- test$ncbi[2]
