...
li_tax <- read.csv("li_taxonomy.csv")

# test <- li_tax %>%
#   group_by(family) %>%
#   summarize(species = paste(old_taxonID_linked_genome_sequence, collapse = '", "'))

unique_fam <- unique(li_tax$family)
species_list <- list()

for (fam in unique_fam) {
  species <- li_tax$species[li_tax$family == fam]
  species_list[[fam]] <- species
}


# use findMRCA of phytools to find the family node OR getMRCA of ape
# trim trees down to the family node
```
################################
